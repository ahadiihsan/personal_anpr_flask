...
def top_hat(image):
    ...
    '''
        ...
        return val [open] - image after being preprocessed
                            ...
                            close is the morphological operation used to
                            close small holes in the image
                            open is the morphological operation used to
                            remove noise
    '''
    ...
    kernel_tophat = cv2.getStructuringElement(
            cv2.MORPH_RECT, (CHARACTER_SIZE[0]*2, 1)
        )
    kenel_close = cv2.getStructuringElement(
            cv2.MORPH_RECT,
            (np.uint8(CHARACTER_SIZE[0]/ACCEPTED_ERROR), 1)
        )
    kernel_open = cv2.getStructuringElement(
            cv2.MORPH_RECT,
            (
                np.uint8(PLATE_SIZE[0] * ACCEPTED_ERROR),
                np.uint8(CHARACTER_SIZE[0] * ACCEPTED_ERROR)
            )
        )
    ...
    if config["save_image"]: cv2.imwrite("./image/img_top_hat1.jpeg", tophat)
    ...
    if config["save_image"]: cv2.imwrite("./image/img_top_hat.jpeg", open)
