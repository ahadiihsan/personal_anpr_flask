...
def normalize_plate(points):
    ...
    '''
        params [points] - array with sum of rows, returned from
                            image_threshold_hor, selects points
                            above the threshold and stores them as
                            plate coordinates
...
def image_threshold_hor(image):
    ...
    '''
        ...
        return val [horizontal] - array with sum of columns from
                                thresholded image, used for
                                horizontal projection, can be
    ...
    cv2.imwrite("./image/img_thres.jpeg", thresh)
